...
window.addEventListener("load", function (e) {
  ...
  class Background {
    constructor(gameWidth, gameHeight) {
      ...
      this.width = 2400;
      this.height = 720;
      this.level = Math.floor(score / 7) + 1;
      this.speed = Math.min(this.level, 7) * 7;
    }
    draw(context) {
  ...
  class Enemy {
    constructor(gameWidth, gameHeight) {
      ...
      this.frameTimer = 0;
      this.frameInterval = 1000 / this.fps;
      this.level = Math.floor(score / 8) + 1;
      this.speed = Math.min(this.level, 8) * 8;
  ...
  function handleEnemies(delaTime) {
  ...
  function toggleFullScreen() {
    if (!document.fullscreenElement) {
      canvas.requestFullscreen().catch((err) => {
        console.log(`Error, can't enable full-screen mode: ${err.message}`);
      });
    } else {
  ...
  function animate(timeStamp) {
    ...
    background.update();
    player.draw(ctx);
    player.update(input, delaTime, enemies);
    handleEnemies(delaTime);
